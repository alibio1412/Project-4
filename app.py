...
if __name__ == '__main__':
    ...
    try:
        ...
        # ingest data to dwh
        df.to_sql('dim_orders_alissa', engine_dwh, if_exists='append', index=False)

        # upload file to hadoop as DWH
        filetime = datetime.now().strftime('%Y%m%d')
        my_file = f'dim_orders_{filetime}_alissa.csv'
        ...
        print('[INFO] Upload Data in HADOOP Success ...')

        # get data from hadoop for create data mart
        print("[INFO] Get Data in HADOOP ...")
        ...
        print('[INFO] Download Data in HADOOP Success and Created file mart')
